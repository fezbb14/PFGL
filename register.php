...
<body>
	...
	<?php
		...
		if (empty($_POST['fname']) || empty($_POST['email']) || empty($_POST['pass']))
			...
			{
			//update the record
			$sql = "INSERT INTO registration(Fname,Lname,Address,Mobno,Email,Pass) VALUES('$_POST[fname]', '$_POST[lname]', '$_POST[address]','$_POST[mobno]','$_POST[email]',SHA('$_POST[pass]'))";
			$result = mysqli_query($conn, $sql) or die( "<br> <br>"."<b>"."<font size=+3>"."Error registration".mysqli_error($conn));
			$numrows = mysqli_affected_rows($conn);
			echo "<br> <br>";
			if ($numrows == 1)
				echo "<b>"."<font size=+3>". "Welcome $_POST[fname]! You have been successfully registrated in our website.";
			else
				echo "<b>"."<font size=+3>"."Registration failed."; 
			}
	?>
	</div>
	</div>
